...
void AWtdiff(const char * filename){
  ...
  TH1D* largeh_r = h2_time->ProjectionY("largeh_r",0,nbinx);
  ...
  */
  ...
  TH2D* hc_tdiff= new TH2D("hc_tdiff", "histo hc_tdiff",nbinx,txmin,txmax,nbiny,tymin,tymax);
   ...
   TH1D* corrh_r = hc_time->ProjectionY("corrh_r",0,nbinx);
...
for(k=0;k<digiTree->GetEntries();k++){
  ...
  if ((0.8*(fit_l->GetParameter(1)) < (amp_max[AMP1]/max) && (amp_max[AMP1]/max) < (3*fit_l->GetParameter(1))) || ((0.8*(fit_r->GetParameter(1)) < (amp_max[AMP2]/max) && (amp_max[AMP2]/max) < (3*fit_r->GetParameter(1)))) ) 
	...
	hc_tdiff->Fill(time[NINO1+LEDi]-time[NINO2+LEDi],(time[NINO1+LEDi]+time[NINO2+LEDi])/2-time[0+CFD]-(hyp_r->Eval(amp_max[AMP2]/max)-hyp_r->GetParameter(0)+hyp_l->Eval(amp_max[AMP1]/max)-hyp_l->GetParameter(0))/2-fit_tdiff->Eval(time[NINO1+LEDi]-time[NINO2+LEDi])+fit_tdiff->GetParameter(0));	
       }
   }
 }//chiudo for k
...
/*#################################################################################################################################*/ 
 ...
 histo_ct = hc_time->ProjectionY("histo_ct",0,nbinx);
 ...
 TF1* iterFitCt = fitGaus(histo_ct,1.7,false,"t");
...
bool ConfrontoTdiff=false;
   ...
   if(ConfrontoTdiff){
     ...
     ConfCanv->cd(2);
     hc_time->GetYaxis()->SetTitle("t_ave-t_MCP [ns]");
     hc_time->GetXaxis()->SetTitle("t_left-t_right [ns]");
     hc_time->Draw("COLZ");
     graph_tc->Draw("P");
     fit_tdiff->Draw("SAME");
