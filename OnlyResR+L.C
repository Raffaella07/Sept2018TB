...
TF1* fitGaus( TH1D* histo, float nSigma, bool addFunc, string type ) {
  ...
  TCanvas* iterCanvas = new TCanvas("iterFitCanvas","",1300,900);
...
float GetSigmaEff( TH1D* histo, string dir) {
  ...
  while( newHisto->Integral() < percentIntegral*integral ) {
    ...
    ControlloSigmaEff= new TCanvas("ControlloSigmaEff","",1300,900);
...
TH2D* AWtdiff(const char * filename){
  ...
  TTree* infoTree = (TTree*)file->Get("info");
  ...
  Float_t rxmin_l,rxmax_l,rxmin_r,rxmax_r,rymin_l,rymax_l,rymin_r,rymax_r,tymin,tymax,txmin,txmax,tymin_c,tymax_c,rymin_lc,rymax_lc,rymin_rc,rymax_rc,MinHodo,MaxHodo;
  bool debug=false;
  bool blind=true;
  Double_t max=0;
  ...
  TF1 *fit_r = new TF1("f_r","landau",0.15,0.9);
  TF1 *fit_l = new TF1("f_l","landau",0.01,0.4);
  ...
  infoTree->SetBranchAddress("tableX", &Xtable );
  ...
  for(k=0;k<digiTree->GetEntries();k++){
    ...
    if(k%100000==0)cout << "FillArray " << k <<"/"<< digiTree->GetEntries()<< endl;
  }
  ...
  */
  ...
  TCanvas* lcheck = new TCanvas("landaucheck","landaucheck plot", 1300, 700);
  lcheck->Divide(3,1);
  lcheck->cd(1)->SetLogy();
  ...
  TH2F* h2_l= new TH2F("h2_l", "histo h2_l",nbinx,rxmin_l,rxmax_l,nbiny,rymin_l,rymax_l);
  TH2F* h2_r= new TH2F("h2_r", "histo h2_r",nbinx,rxmin_r,rxmax_r,nbiny,rymin_r,rymax_r);
  ...
  TCanvas* wf_c =new TCanvas("wf","Plot wf",1700,900);
  ...
  */
  ...
  TCanvas* corrg = new TCanvas("corr_g","plot corr_g", 900, 800);
  ...
  TCanvas* CanvAmpRVsX = new TCanvas("AmRVsX","",1200,800);
  AmpRVsX->Draw("COLZ");
