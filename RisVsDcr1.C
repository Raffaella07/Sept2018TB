...
/*void compareTGraphs(Float_t DCR, TGraphErrors* graph,TF1* fit,Int_t i){
  Int_t nfiles = 4;
  TGraphErrors* fitg[nfiles];
  fitg[i]=graph
...
//####################################################################################################################################################################################################################### 
void GaussianAmpWalk(TFile* file,Float_t index, Float_t ranges[][2],Float_t* NAWRes,Float_t* errNAWRes,TF1* newfit[][2],Int_t ind){
...
void plotWF_lsig(TFile* file,Float_t* Resol,Float_t* errResol,Float_t index,Float_t* Resolc,Float_t* errResolc,Float_t ranges[][2]){
  ...
  digiTree->SetBranchAddress("amp_max",&amp_max);
  digiTree->SetBranchAddress("time",&t);
  ...
  hl_amp->Fit("f_l","RQ0");
   ...
   TH2D* h2_m= new TH2D("h2_m", "histo h2_m",nbinx,rxmin,rxmax,nbiny,rymin_l,rymax_l);
  ...
  for(k=0;k<digiTree->GetEntries();k++){
	...
	h2_m->Fill(t[1+LEDi]-t[2+LEDi],(t[1+LEDi]+t[2+LEDi])/2-t[0]);
      }
  ...
  histotemp_m=h2_m->ProjectionY(((string)"h2_tprojY"+to_string((Int_t)index)).c_str(),0,nbinx);
  ...
  histotemp_m->Fit(((string)"g_m"+to_string((Int_t)index)).c_str(),"0R");
  ...
  *errResol = g_m->GetParError(2);
  ...
  TH2D* hc_tdiff= new TH2D("hc_tdiff", "histo hc_tdff",nbinx,rxmin,rxmax,nbiny,rymin_l,rymax_l);
  ...
  fit_tdiff->Draw("same");
     ...
     if (0.8*(fit_l->GetParameter(1)) < (amp_max[4]/max) && (amp_max[4]/max) < (3*fit_l->GetParameter(1)) && amp_max[0]/max > 0.4 && amp_max[0]/max < 0.75)
      {   
	hc_tdiff->Fill(t[1+LEDi]-t[2+LEDi],(t[1+LEDi]+t[2+LEDi])/2-t[0]-fit_tdiff->Eval(t[1+LEDi]-t[2+LEDi])+fit_tdiff->GetParameter(0));
      }//chiudo for k
...
void Ris(TFile* file,Float_t* Resol,Float_t* errResol,Float_t* ResolHisto,Float_t index,Float_t* NAWRes,Float_t* errNAWRes, Float_t ranges[][2],TF1* oldfit[][2], Int_t ind){
  ...
  TH2D* hc_tdiff= new TH2D("hc_tdiff", "histo hc_tdiff",nbinx,txmin,txmax,nbiny,tymin_c,tymax_c);
   ...
   //  ProjAWC(hc_r,"corrOAW",nbinx,index,"NAW");
   // ProjAWC(h2_r,"uncorrOAW",nbinx,index,"OAW");
   ProjAWC(h2_t,"uncorrOAW_t",nbinx,index,"OAW");
   ProjAWC(hc_t,"corrOAW_t",nbinx,index,"OAW");
   ProjAWC(h2_r,"uncorrOAW_r",nbinx,index,"OAW");
   ProjAWC(hc_r,"corrOAW_r",nbinx,index,"OAW");
...
void RisVsDcr1(string conf){
  ...
  /*  TLatex* tex[nfiles];
   ...
   //nocorr_sigma->cd();
    ...
    csigma->SetMarkerSize(1.3);
    graph->SetMarkerSize(1.3);
    graph->SetMarkerStyle(20);
    csigma->SetMarkerColor(kBlue);
    csigma->SetLineColor(kBlue);
    NAWGraph->SetMarkerStyle(24);
    NAWGraph->SetMarkerSize(1.3);
    NAWGraph->SetMarkerColor(kRed);
    NAWGraph->SetLineColor(kRed);
    ...
    l5l->AddEntry(fitsold[0][0],(to_string((Int_t)DCR[0])+" DCR").c_str(),"L");
    ...
    for(i=1;i<nfiles;i++){
      ...
      l5l->AddEntry(fitsold[i][0],(to_string((Int_t)DCR[i])+" DCR").c_str(),"L");
    ...
    l5r->AddEntry(fitsold[0][1],(to_string((Int_t)DCR[0])+" DCR").c_str(),"L");
    ...
    for(i=1;i<nfiles;i++){
      ...
      l5r->AddEntry(fitsold[i][0],(to_string((Int_t)DCR[i])+" DCR").c_str(),"L");
    ...
    n5l->AddEntry(fitsnew[0][0],(to_string((Int_t)DCR[0])+" DCR").c_str(),"L");
    ...
    for(i=1;i<nfiles;i++){
      ...
      n5l->AddEntry(fitsnew[i][0],(to_string((Int_t)DCR[i])+" DCR").c_str(),"L");
    ...
    n5r->AddEntry(fitsnew[0][1],(to_string((Int_t)DCR[0])+" DCR").c_str(),"L");
    ...
    for(i=1;i<nfiles;i++){
      ...
      n5r->AddEntry(fitsnew[i][1],(to_string((Int_t)DCR[i])+"DCR").c_str(),"L");
