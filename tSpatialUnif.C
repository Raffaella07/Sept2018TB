...
void plotWF_time(const char * filename){
 ...
 for(k=0;k<nbinx;k++){
  ...
  for(k=0;k<digiTree->GetEntries();k++){
    ...
    e->Fill(X[0],Y[0],amp_max[AMP1]/max);
   ...
   for(k=0;k<digiTree->GetEntries();k++){
      ...
      if (0.8*(fit_l->GetParameter(1)) < (amp_max[AMP1]/max) && (amp_max[AMP1]/max) < (3*fit_l->GetParameter(1)))
      {
	 h3Y_l->Fill(X[0],time[NINO1+LEDi]-time[PTK1+CFD]);
      }
    if (0.8*(fit_r->GetParameter(1)) < (amp_max[AMP2]/max) && (amp_max[AMP2]/max) < (3*fit_r->GetParameter(1)) ){
	h3Y_r->Fill(X[0],time[NINO2+LEDi]-time[PTK1+CFD]);
    }
    if ((0.8*(fit_l->GetParameter(1)) < (amp_max[AMP1]/max) && (amp_max[AMP1]/max) < (3*fit_l->GetParameter(1))) || ( 0.8*(fit_r->GetParameter(1)) < (amp_max[AMP2]/max) && (amp_max[AMP2]/max) < (3*fit_l->GetParameter(1)) ) ) h3Y_t->Fill(X[0],(time[NINO1+LEDi]+time[NINO2+LEDi])/2-time[PTK1+CFD]);//chiudo for k
    }
   }
